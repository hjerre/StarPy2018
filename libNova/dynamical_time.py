...
class year_TD:
    def __init__(self, year = 0, TD = 0.0):
        self.year = year
        self.TD = TD
...
def get_dynamical_diff_sh1( JD : float ) -> float:
...
def get_dynamical_diff_sh2( JD : float ) -> float:
...
def get_dynamical_diff_table( JD : float ) -> float:
    TD = 0
...
def get_dynamical_diff_near( JD : float ) -> float:
    TD = 0
    # TD for 1990, 2000, 2010
    delta_T = [56.86, 63.83, 70.0]
...
def get_dynamical_diff_other( JD : float ) -> float:
    a = (JD - 2382148.0)
    a *= a
...
# def ln_get_dynamical_time_diff( JD  ) -> float
# param JD          Julian Day
# returns TD

# Calculates the dynamical time (TD) difference in seconds (delta T) from universal time.


def ln_get_dynamical_time_diff( JD : float ) -> float:
    # check for date < 948 A.D.
    if JD < 2067314.5:
        # Stephenson and Houlden
        TD = get_dynamical_diff_sh1(JD)
    elif JD >= 2067314.5 and JD < 2305447.5:
        # check for date 948..1600 A.D. Stephenson and Houlden
        TD = get_dynamical_diff_sh2(JD)
    elif JD >= 2312752.5 and JD < 2448622.5:
        # check for value in table 1620..1992  interpolation of table
        TD = get_dynamical_diff_table(JD)
    elif JD >= 2448622.5 and JD <= 2455197.5:
